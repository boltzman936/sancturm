...
export function UploadResourceForm({
  ...
  branchId: string;
  resourceType: NotesOrLab;
}) {
  const { data: allSubjects } = useSubjects(branchId);
  // Lab uploads only ever apply to the subjects that actually have a
  // lab component — no point offering "Human Values" as a lab subject.
  const subjects =
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    upload.mutate(
      { branchId, subjectId, section: "notes_lab", resourceType, title, description, file },
      {
        onSuccess: () => {
    ...
    >
      ...
      {upload.isSuccess && (
        <p className="font-mono text-xs text-terminal-blue">
          Submitted — a CR/admin will review it before it appears here.
        </p>
      )}
      {upload.isError && (
