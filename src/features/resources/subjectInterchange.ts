...
// which SPECIALIZATION's subject list Core/AIML/AIDS actually use for
// that one semester. Identity function everywhere else: every other
// term, and every other specialization (Cyber Security — confirmed
// independent — and anything outside CSE, which has no specialization
// concept at all) is completely unaffected regardless of the toggle's
// value.
//
...
// upload while interchanged is still a genuine Core resource; its
// subject_id just points at one of AIDS's subject rows instead of
// Core's own, because that's the subject list currently active for
// Core at that semester. This is what "Subjects, Labs, PYQs, any
// subject-linked resource" resolving to the active structure actually
// means in this schema — resources are already subject-linked via
// subject_id, so swapping which subject rows a specialization resolves
// against IS swapping the whole subject-linked structure, without
// duplicating a single row or moving any resource between branches.
//
...
export function resolveSubjectSpecializationName(
  requestedSpecializationName: string,
  termSlug: string | null,
  interchangeActive: boolean
): string {
  if (!interchangeActive || termSlug !== INTERCHANGE_TERM_SLUG) return requestedSpecializationName;
